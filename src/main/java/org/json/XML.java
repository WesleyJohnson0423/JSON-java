...
public class XML {
    ...
    /**
     *      Milestone 3 - 6 parameters parse function
     * */
    /**
     * Scan the content following the named tag, attaching it to the context with optional key transformation.
     *
     ...
     * @param keyTransformer Optional key transformer function (can be null)
     * @return true if the close tag is processed.
     * @throws JSONException Thrown if any parsing error occurs.
     */
    private static boolean parse(XMLTokener x, JSONObject context, String name, XMLParserConfiguration config,
                                 int currentNestingDepth, KeyTransformer keyTransformer) throws JSONException {
        char c;
        int i;
        JSONObject jsonObject = null;
        String string;
            ...
            if (c == '-') {
                ...
                if ("CDATA".equals(token)) {
                    if (x.next() == '[') {
                        string = x.nextCDATA();
                        if (string.length() > 0) {
                            // Apply key transformation to CDATA tag name
                            String cdataKey = keyTransformer != null ?
                                    keyTransformer.transform(config.getcDataTagName()) :
                                    config.getcDataTagName();
                            context.accumulate(cdataKey, string);
                        }
                        return false;
                    }
                }
                ...
                if (token instanceof String) {
                    ...
                    if (token == EQ) {
                        ...
                        if (config.isConvertNilAttributeToNull()
                            ...
                            // Apply key transformation to attribute names
                            String transformedKey = keyTransformer != null ?
                                    keyTransformer.transform(string) : string;

                            Object obj = stringToValue((String) token);
                            if (obj instanceof Boolean) {
                                jsonObject.accumulate(transformedKey,
                                        config.isKeepBooleanAsString()
                                                ? ((String) token)
                                                : obj);
                            } else if (obj instanceof Number) {
                                ...
                                jsonObject.accumulate(transformedKey, stringToValue((String) token));
                            }
                        }
                        token = null;
                    } else {
                        // Apply key transformation to attribute names without values
                        String transformedKey = keyTransformer != null ?
                                keyTransformer.transform(string) : string;
                        jsonObject.accumulate(transformedKey, "");
                    }
                    ...
                    // Apply key transformation to tag names
                    String transformedTagName = keyTransformer != null ?
                            keyTransformer.transform(tagName) : tagName;
                    ...
                    for (;;) {
                        ...
                        if (token == null) {
                            ...
                            if (string.length() > 0) {
                                ...
                                if(xmlXsiTypeConverter != null) {
                                    jsonObject.accumulate(cdataKey,
                                            stringToValue(string, xmlXsiTypeConverter));
                                } else {
                                    Object obj = stringToValue((String) token);
                                    if (obj instanceof Boolean) {
                                        ...
                                        jsonObject.accumulate(cdataKey, stringToValue((String) token));
                                    }
                                }
                            }
                            ...
                            // Recursive call with key transformer
    ...
    // ============ Milestone 3: Key Transformer Support ============

    public static JSONObject toJSONObject(Reader reader, KeyTransformer keyTransformer) throws JSONException {
        return toJSONObject(reader, false, keyTransformer);
    }

    public static JSONObject toJSONObject(Reader reader, boolean keepStrings, KeyTransformer keyTransformer) throws JSONException {
        XMLParserConfiguration config = keepStrings ? XMLParserConfiguration.KEEP_STRINGS : XMLParserConfiguration.ORIGINAL;
        return toJSONObject(reader, config, keyTransformer);
    }

    public static JSONObject toJSONObject(Reader reader, XMLParserConfiguration config, KeyTransformer keyTransformer) throws JSONException {
        JSONObject jo = new JSONObject();
        XMLTokener x = new XMLTokener(reader, config);
        while (x.more()) {
            x.skipPast("<");
...
// ============ Milestone 5: 异步API ============
    ...
    public static CompletableFuture<Void> toJSONObjectAsync(Reader reader,
    ...
    public static CompletableFuture<JSONObject> toJSONObjectAsync(Reader reader, KeyTransformer keyTransformer) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                return toJSONObject(reader, keyTransformer);
            } catch (JSONException e) {
                throw new RuntimeException(e);
            }
        });
